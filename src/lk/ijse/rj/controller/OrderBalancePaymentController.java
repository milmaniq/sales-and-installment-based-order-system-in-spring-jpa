...
public class OrderBalancePaymentController implements Initializable {
    ...
    public OrderBalancePaymentController() {
        orderBalancePaymentBO = AppInitializer.context.getBean(OrderBalancePaymentBO.class);
    ...
    private void btnAdd_OnAction(ActionEvent event) {
        if (new BigDecimal(lblBalance.getText()).compareTo(BigDecimal.ZERO) < 0) {
            return;
        }
    ...
    private void btnRemove_OnAction(ActionEvent event) {
        ...
        OrderBalancePaymentDTO orderBalancePaymentDTO = new OrderBalancePaymentDTO(selectedItem.getInc(),
